...
def transform_structs_with_cast_and_keep(df, array_column, target_data_types):
    ...
    for field in struct_type.fields:
        if field.name in target_data_types:
            cast_exprs.append(col(f"{array_column}.{field.name}").cast(target_data_types[field.name]).alias(field.name))
        else:
            cast_exprs.append(col(f"{array_column}.{field.name}").alias(field.name))

    # Apply the transformation using `withColumn` and `transform`
    df_transformed = df.withColumn(
        array_column,
        expr(f"transform({array_column}, x -> struct({', '.join([f'cast(x.{field.name} as {target_data_types[field.name].simpleString()}) as {field.name}' if field.name in target_data_types else f'x.{field.name} as {field.name}' for field in struct_type.fields])}))")
    )
